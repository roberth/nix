...
protected:
     ...
     */
    std::shared_ptr<TracingReplayObject>
    makeReplayObject(const std::vector<trace::TraceEntry> & trace, const trace::QueryIndex & index, uint64_t valueNum)
    {
        return std::make_shared<TracingReplayObject>(*store, trace, index, valueNum, [this]() -> ref<Object> {
...
TEST_F(TracingReplayTest, FallbackToInnerOnMiss)
{
    ...
    auto interpreter = make_ref<Interpreter>(makeState());
    auto replay = std::make_shared<TracingReplayObject>(*store, trace, index, 999, [&]() -> ref<Object> {
        innerCalled = true;
        return interpreter->evalExpr("42", state->rootedPath(CanonPath::root));
    });
