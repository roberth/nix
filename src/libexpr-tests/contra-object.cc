...
{
    auto obj = std::make_shared<ContraObject>("0", mockResolver({{"getType:0", trace::ResultType{"int"}}}));
    EXPECT_EQ(obj->getType(), nInt);
}

TEST(ContraObjectTest, GetInt)
{
    auto obj = std::make_shared<ContraObject>("0", mockResolver({{"getInt:0", trace::ResultInt{42}}}));
    EXPECT_EQ(obj->getInt().value, 42);
}

TEST(ContraObjectTest, GetString)
{
    auto obj = std::make_shared<ContraObject>("0", mockResolver({{"getString:0", trace::ResultString{"hello"}}}));
    EXPECT_EQ(obj->getStringIgnoreContext(), "hello");
}

TEST(ContraObjectTest, GetBool)
{
    auto obj = std::make_shared<ContraObject>("0", mockResolver({{"getBool:0", trace::ResultBool{true}}}));
...
{
    auto obj = std::make_shared<ContraObject>("0", mockResolver({{"getAttr:0", trace::ResultMaybeType{std::nullopt}}}));
