...
static AmbientQueryFn mockResolver(std::map<std::string, trace::ResultVariant> responses)
{
    return [responses = std::move(responses)](int objectId, const trace::QueryVariant & q) -> AmbientQueryResult {
        std::string key = std::visit(
            [&](const auto & query) -> std::string { return std::string(query.tag) + ":" + std::to_string(objectId); },
            q);
...
{
    auto obj = std::make_shared<AmbientObject>(0, mockResolver({{"getType:0", trace::ResultType{"int"}}}));
    EXPECT_EQ(obj->getType(), nInt);
}

TEST(AmbientObjectTest, GetInt)
{
    auto obj = std::make_shared<AmbientObject>(0, mockResolver({{"getInt:0", trace::ResultInt{42}}}));
    EXPECT_EQ(obj->getInt().value, 42);
}

TEST(AmbientObjectTest, GetString)
{
    auto obj = std::make_shared<AmbientObject>(0, mockResolver({{"getString:0", trace::ResultString{"hello"}}}));
    EXPECT_EQ(obj->getStringIgnoreContext(), "hello");
}

TEST(AmbientObjectTest, GetBool)
{
    auto obj = std::make_shared<AmbientObject>(0, mockResolver({{"getBool:0", trace::ResultBool{true}}}));
...
{
    auto obj = std::make_shared<AmbientObject>(0, mockResolver({{"getAttr:0", trace::ResultMaybeType{std::nullopt}}}));
