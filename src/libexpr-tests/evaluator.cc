...
protected:
    ...
    /** Unique path per evalExpression call to avoid SQLite lock contention. */
    ...
    {
        if (isInterpreter()) {
            return evaluator->evalExpr(expr, evaluator->getEvalState().rootedPath(CanonPath::root));
        }
    ...
    {
        return evaluator->evalExprLazy(expr, evaluator->getEvalState().rootedPath(CanonPath::root));
    }
    ...
    void runTestWithCaching(TestBody body)
    {
        if (isCoarseCacheWithPersistence()) {
            ...
            for (auto & [run, label] : std::initializer_list<std::pair<int, const char *>>{
                     {1, "Cold cache run"},
                     {2, "Warm cache run"},
                 }) {
                SCOPED_TRACE(label);
...
EVALUATOR_TEST(Object_getType_nThunk, {
    // CoarseEvalCache always forces values, so it never exposes thunks
    if (!isInterpreter()) {
        GTEST_SKIP() << "Thunk testing only applicable for Interpreter";
    }
    ...
    // getTypeLazy returns nThunk without forcing; getType forces and returns actual type
...
EVALUATOR_TEST(Object_getType_nExternal, {
    // No Nix syntax creates external values; would need internal state access.
