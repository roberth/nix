    ...
    if (auto r = lookupResult(trace::QueryGetStringWithContext{valueNum})) {
        ...
        for (const auto & elem : ctx) {
            const StorePath & path = std::visit(
                // TODO (after prototyping): factor into NixStringContextElem method
                overloaded{
...
{
    debug("replay miss: getPath (v=%d) not cached", valueNum);
...
{
    debug("replay miss: defeatCache (v=%d)", valueNum);
