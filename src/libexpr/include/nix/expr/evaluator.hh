...
public:
     ...
     *
     * Returns a `RootedPath` so the `SourceRoot`'s kind and
     * `unpinnedId` survive cache round-trips — the eval cache's
     * identifier-keyed storage keys on `SourceRoot` identity, not on
     * a bare accessor pointer. A `SourcePath` here would drop that
     * information at the Object boundary and force every path-typed
     * cache hit through forceValue.
     */
    virtual RootedPath getPath() = 0;
     ...
     */
    virtual std::shared_ptr<const struct ArgCell> getProxyArgCell() const
    {
        return nullptr;
    }
     ...
     * (`TracingReplayEvaluator::resolveStateHash`'s cell-chain branch).
     *
...
public:
     ...
     * @param basePath The base path for relative imports
     */
    virtual ref<Object> evalExprLazy(const std::string & expr, const RootedPath & basePath) = 0;
