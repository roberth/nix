...
ref<Object> CoarseEvalCache::evalFile(const RootedPath & path, const std::string & displayPath)
{
    debug("CoarseEvalCache::evalFile falling back to direct evaluation for '%s'", displayPath);
    return inner->evalFile(path, displayPath);
}

ref<Object> CoarseEvalCache::evalExpr(const std::string & expr, const RootedPath & basePath)
{
    debug("CoarseEvalCache::evalExpr falling back to direct evaluation");
    return inner->evalExpr(expr, basePath);
}

ref<Object> CoarseEvalCache::evalExprLazy(const std::string & expr, const RootedPath & basePath)
{
