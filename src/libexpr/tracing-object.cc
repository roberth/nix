...
/* TODO: absorb apply-result observations into the proxy's argScope
   cell, so the cell's contentId() differentiates cb invocations
   whose body behaviour differs. Disabled for now — needs a root
   cell mechanism (cache's evalFile result with a cell incorporating
   the cache's Q hash and chained-in file_hash facts) to give
   sibling cb cells distinct CDIs across different cache invocations
   and propagate inner-mod file changes to the apply Q. */
template<typename Q>
