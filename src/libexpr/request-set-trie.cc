...
/* ─────────────────────────────────────────────────────────────────────
   MutableNode — Pimpl body
   ───────────────────────────────────────────────────────────────────── */

struct MutableNode::Body
{
    std::unordered_set<Hash> members;
...
MutableNode::MutableNode(FrozenNodePtr root)
    : body(std::make_unique<Body>())
{
    auto seed = root->allMembers();
    body->members.insert(seed.begin(), seed.end());
    body->cachedFrozen = root.get_ptr();
}

void MutableNode::insert(const Hash & h)
{
    if (body->members.insert(h).second)
        body->cachedFrozen = nullptr;
}

bool MutableNode::contains(const Hash & h) const noexcept
{
    return body->members.count(h) > 0;
}

size_t MutableNode::size() const noexcept
{
    return body->members.size();
}

FrozenNodePtr MutableNode::freeze(FrozenNodeCache & cache)
{
    if (body->cachedFrozen)
        return FrozenNodePtr(body->cachedFrozen);
    std::vector<Hash> ms(body->members.begin(), body->members.end());
    auto r = cache.internSet(std::move(ms));
    body->cachedFrozen = r.get_ptr();
    return r;
}
