    ...
    : _state(std::make_unique<Sync<State>>())
{
    ...
    state->getQuery.create(
        state->db,
        "SELECT nodeHash, queryHash, afterHash, structuralParent, isEnvironment FROM Queries WHERE nodeHash = ?");
    ...
    state->getChildRequests.create(state->db, "SELECT DISTINCT request FROM Responses WHERE afterHash = ?");
    ...
    state->selectChildQueries.create(
        state->db,
        "SELECT nodeHash, queryHash, afterHash, structuralParent, isEnvironment FROM Queries WHERE afterHash = ?");
