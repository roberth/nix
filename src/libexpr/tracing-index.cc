        ...
        validator)
{
    ...
    walk = [&](const NodeHash & current) -> std::optional<ResultNode> {
        ...
        tracingCacheLog(
            "findResult[%d] at %s: %zu results, %zu queries (target=%s)",
            walkDepth,
            hashPrefix(current),
            results.size(),
            queries.size(),
            hashPrefix(queryNodeHash));

        for (const auto & r : results) {
            bool isTarget = r.queryNodeHash && *r.queryNodeHash == queryNodeHash;
            tracingCacheLog(
                "findResult[%d]   result %s qnHash=%s %s",
                walkDepth,
                hashPrefix(r.nodeHash),
        ...
        for (const auto & child : queries) {
            ...
            if (!payload) {
                tracingCacheLog(
                    "findResult[%d]   depth=%d query %s: no payload",
                    walkDepth,
                    child.depth,
                    hashPrefix(child.nodeHash));
                continue;
            }

            auto evResults = childResults(child.nodeHash);
            tracingCacheLog(
                "findResult[%d]   depth=%d query %s: %zu event results",
                walkDepth,
                child.depth,
                hashPrefix(child.nodeHash),
                evResults.size());
            ...
            for (const auto & eventResult : evResults) {
                ...
                if (!valid) {
                    try {
                        ...
                        tracingCacheLog(
                            "findResult[%d]     event result %s: INVALID query=%s recorded=%s",
                            walkDepth,
                            hashPrefix(eventResult.nodeHash),
                            qj.dump().substr(0, 120),
                            rj.dump().substr(0, 80));
                    } catch (...) {
                        tracingCacheLog(
                            "findResult[%d]     event result %s: INVALID (dump failed)",
                            walkDepth,
                            hashPrefix(eventResult.nodeHash));
                    }
                } else {
                    tracingCacheLog(
                        "findResult[%d]     event result %s: VALID", walkDepth, hashPrefix(eventResult.nodeHash));
                }
                if (valid) {
                    walkDepth++;
