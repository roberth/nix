...
static ObjectType stringToObjectTypeContra(const std::string & type)
{
    if (type == "set")
        return nAttrs;
    if (type == "list")
...
RootedPath ContraObject::getPath()
{
    ...
    // ContraObject can't reconstruct a full RootedPath from just a
...
ObjectType ContraObject::getType()
{
    ...
    return stringToObjectTypeContra(r->type);
}
