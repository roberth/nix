...
public:
    ...
    ~NullTraceSink() override;

    void log(const nlohmann::json &) override {}
};

NullTraceSink::~NullTraceSink() = default;
...
static void prim_cache(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    /* lazy-paths: evalFile/evalExpr now take RootedPath. Use the inner
       state's rootFSRoot (System) so the inner accessor's reads are
       admitted as system-environment dependencies for cache
       invalidation. */
