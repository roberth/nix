...
struct CmdEvalCacheInspect : Command
{
    ...
    void run() override
    {
        ...
        std::cerr << "Parsed hash: " << nodeHash.to_string(HashFormat::Base16, false) << " (" << nodeHash.hashSize
                  << " bytes)\n";
        ...
        if (auto q = index.getQuery(nodeHash)) {
            ...
            std::cout << "AfterHash: "
                      << (q->afterHash ? q->afterHash->to_string(HashFormat::Base16, false) : "NULL (root)") << "\n";
            std::cout << "StructuralParent: "
                      << (q->structuralParent ? q->structuralParent->to_string(HashFormat::Base16, false) : "NULL")
                      << "\n";
            ...
            std::cout << "QueryNodeHash: "
                      << (r->queryNodeHash ? r->queryNodeHash->to_string(HashFormat::Base16, false) : "NULL") << "\n";
...
private:
    void printTree(TracingIndex & index, const NodeHash & nodeHash, int depth)
    {
        ...
        if (auto q = index.getQuery(nodeHash)) {
            ...
            std::cout << indent(depth) << "R " << nodeHash.to_string(HashFormat::Base16, false).substr(0, 12) << " "
