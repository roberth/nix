...
#   6 (original) → 8 (after multi-root apply-path observations
#   land at d1) → 12 (after per-flush Asks edges land — each Q's
#   recorded chain has one Asks edge per writer flush, principles
#   3/5/7, so the walker visits multiple Asks edges per Q's
#   lookup instead of one whole-remaining edge).
