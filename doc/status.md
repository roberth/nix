...
**cb-two-sibling-distinct-callbacks — remaining failure.** Cold correctly emits distinct QCAs (2023aaf0427d for sibling A with response 42; d03d71e0189b for sibling B with response 99). DB has distinct Terminals under distinct chain traces (589ca81923a0 vs 50da732782bd) for the enclosing Q=641d026a7575. Warm's problem is walker-side: trace-continuing starts sibling B's walk at session envCur = cur_A (from A's earlier walk in the same shared innerReplayEval), immediately hits `getTerminal(Q, cur_A)` = Terminal_A, returns A's result. Never walks the chain that would dispatch QCA-B and reach Terminal_B.

Per design, sibling discrimination for cb-apply results happens at the QCA queryHash level (different obsSet content), not via Q evolution — ApplyResultSubject's state hash is essentially structural under combineArgAncestries propagation. The fix is not stamping/argAncestry (my earlier R4 hypothesis was based on misreading a mechanical docstring rename: c68bd4e38 swept "apply boundary" → "cb-apply" wholesale in doc/comment text, but the function was always for every apply). It's whether trace-continuing should apply across separate `<cached-fn>` invocations — sibling B is a semantically fresh cache-fn call, not a continuation of sibling A's chain, so session envCur shouldn't carry over.

The test's own header comment prescribes the fix: "warm's second-sibling walk tries outgoing Asks at envCur first. Under matching-until-divergence, the wrong sibling's chain has extensions from that cur — warm dispatches them under its own live cell chain and gets responses that match cold's b-recording, advancing to b's Terminal." That's a targeted walker-algorithm change in `decisionGraph.walk`: at each cur, try outgoing Asks first; only return the Terminal at cur when no Ask advances live. Under matching-until-divergence this is safe — if the walker's env doesn't match any outgoing Ask's chain, it correctly falls back to the Terminal at cur.
