...
Two directions: (a) reset walker session state per `<cached-fn>.impl` invocation; (b) trace-continuing verifies that session cur corresponds to a valid entry point for the Q being walked, not just any recorded cur. (a) is the cleaner semantic — each cached function application is its own trace.
