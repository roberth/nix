...
**cb-two-sibling-distinct-callbacks — remaining failure.** Cold correctly emits distinct QCAs (2023aaf0427d for sibling A with response 42; d03d71e0189b for sibling B with response 99). Sibling discrimination for cb-apply results happens at the QCA queryHash level (different obsSet content → different QCA reqHash).

Diagnosis (2026-07-23): warm's sibling B walk ends up at the same `(Q, cur)` as sibling A and returns Terminal_A. Under the design, Ask and Terminal form a deterministic decision tree — at each `(Q, cur)`, either there's a Terminal (that IS the answer) or Asks to dispatch. Two siblings landing at the same `(Q, cur)` with different intended answers is the wrong state to be in; they should have DIVERGED before reaching it. The divergence should manifest as either distinct `Q_final` (Q evolution) or distinct `cur` at the point where the Terminal is queried.

Empirically both siblings land at same Q_final for Q=641d026a7575 (Q evolution didn't discriminate them) and same startCur (session-cumulative envCur inherited from sibling A's terminal). The walker then correctly returns the recorded Terminal_A — because at `(Q_A_final, cur_A)`, that's the recorded answer. The bug is upstream: sibling B shouldn't have arrived at that same `(Q, cur)`.

Where the divergence should have happened but didn't: not yet identified. Retracted the "try Asks before Terminal" walker-side speculation — that would violate the deterministic-decision-tree design (multiple Terminals at same key would be ambiguous). Real fix has to make sibling B reach a distinct `(Q, cur)` on its own — either through Q evolution reflecting the QCA divergence, or through the observation trajectory diverging before Terminal.
