...
**Walker-side gap**: `TracingReplayEvaluator::apply` builds a `TracingReplayObject` with `applyResultSubject={fn, arg}` for the inner apply's result, but there's no counterpart for the nested cb-apply that cold's `OuterApply::run` wraps. Cold's stamped `from` fields on the cb-apply chain reference evolved state hashes of `applyResultSubject.fn = fnSubject`, and warm's cell chain has no live Object at those evolved hashes. Tests where PIR's fixed-state shortcut was masking this gap (single-callback tests like `cb-forcedness-independence`, `cb-stats-sidecar-baseline`) now fall back to inner. The gap is the natural next step: walker needs to construct a synthetic wrapper mirroring cold's `OuterApply::run` wrapper — enough for `resolveStateHash` to compute the same evolved hashes cold stamped.
