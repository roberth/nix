...
**R4 (new): argAncestry asymmetry between emit stamping and Q evolution.** For cb-two-sibling-distinct-callbacks (still failing under baseline): the outer inner-apply Q (Q=641d026a7575) uses `applyArgAncestry = combineArgAncestries(innerFn.argAncestry, arg.argAncestry) = SHA(...)` (a SHA-derived hash) for Q evolution. QCA emit stamps its `from` field via `logOuterObservation` using `applyArgAncestry = cell.argAncestryHex = callArgAncestry` (the raw cache-call XOR seed). These are different hashes by construction. Under `stateHashAt` for composite subjects, "argAncestry propagates recursively through constituents' state hashes" (subject-id.hh:129-139) — so arg(1)'s state hash under Q evolution's `combinedArgAncestry` has a different trajectory than arg(1)'s state hash under emit's `callArgAncestry`. QCA observation stamped under callArgAncestry has `fromHash = arg(1)@callArgAncestry`; Q evolution's fold looks for `fromHash == arg(1)@combinedArgAncestry`; no match; no fold; Q doesn't evolve; sibling siblings collapse at same Q_final. Design tension: design says composite subjects propagate argAncestry through constituents (favoring emit-side adjust to use enclosing Q's argAncestry), but the walker's cell chain resolves proxies under their own inherent argAncestry (favoring emit-side keeping callArgAncestry). A cross-argAncestry stamping change (`stampArgAncestry = activeQueryStack.back().fromSubjectArgAncestry`) plus skipping the wrapper's own Q push regressed 3 tests because warm couldn't resolve the newly-stamped hashes. Real fix needs coordinated writer + walker changes — probably making the walker resolve subjects at contextual argAncestries via the K-loop, or reframing composite-subject argAncestry composition so constituents keep their own scope.
