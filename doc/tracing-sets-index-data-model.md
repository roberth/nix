...
Learning inserts each derived intersection as a new Binding and then evicts any pre-existing Bindings strictly subsumed by it (same queryHash, same responseHash, superset precondition). This keeps the table from growing without bound while still enabling hits in narrower contexts — the synthetic walk collapses 6 Bindings → 3 after `compact-all`. Orphan PreconditionSets and SetResponses left behind by the eviction are then dropped by `runGC` and the database is `VACUUM`ed.
